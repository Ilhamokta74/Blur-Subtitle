...
// Ubah detik jadi teks waktu ringkas, mis: 5 -> "5d", 75 -> "1m 15d", 3700 -> "1j 1m 40d"
        ...
        ff.stderr.on('data', (d) => {
            ...
            if (matches && matches.length) {
                ...
                if (duration > 0 && onProgress) {
                    ...
                    // ETA dihitung dari kecepatan proses SEBENARNYA (bukan cuma persen video),
                    // jadi makin lama makin akurat dan makin kecil seiring progress bertambah.
                    const realElapsedSec = (Date.now() - startTime) / 1000;
                    let etaSeconds = null;
                    if (percent > 0.5) {
                        etaSeconds = (realElapsedSec / percent) * (100 - percent);
                    }
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            if (type === 'image') {
                ...
                // Video: tampilkan progress bar + estimasi waktu tersisa, update live di baris yang sama
                console.log(`⏳ ${label}`);
                const onProgress = (percent, etaSeconds) => {
                    const etaText = etaSeconds === null || etaSeconds === undefined
                        ? 'menghitung...'
                        : `sisa ~${formatDuration(etaSeconds)}`;
                    const line = `   ${renderProgressBar(percent)}  ${etaText}   `;
                    if (isTTY) {
